            ...
            switch (msg)
            {
                case uCallbackMessage:
                    switch (lParam.ToInt32())
                    {
                        ...
                        case WM_RBUTTONUP:
                            Icon icon = new Icon(new MemoryStream(Properties.Resources.RoboSync));
                            if (MessageBox.Show("Exit RoboSync", "RoboSync", MessageBoxButton.YesNo, MessageBoxImage.Stop, MessageBoxResult.Yes, MessageBoxOptions.DefaultDesktopOnly) == MessageBoxResult.Yes)
                            {
                                App.CanClose = true;
                                window.Close();
                            }
